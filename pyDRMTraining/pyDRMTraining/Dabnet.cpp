...
void CDabnet::ProcessIdentificationDataForLaguerreTraining()
{
	...
	int i, j, k, n;
	int istart, iset, npair;
	int nstate_j;
	int npoint = pid_data->npair;
	int nset = pid_data->nset;
	...
	for (iset=0; iset<nset; iset++)
	{
		...
		prow_id = ppdata_id[istart];
		for (j=0; j<ninput; j++)
			lss_list[j].InitStateVectorToSteadyState(prow_id[j]);
		//define u(k) as the input value between k and k+1 time interval [k,k+1)
		//note that each row in id_data is a pair of u(k) and y(k). x(k+1) is obtained from x(k) and u(k) by x(k+1) = A*x(k)+B*u(k)
		//The ANN mapping is x(k) to y(k)
		//The last point of input u, u(npoint-1) is not used.  The first point of u, u(0) is used to calculate steady state x, x(0) and is also used to calculate x(1)
		for (i=0; i<npair; i++)
		{
			prow_id = ppdata_id[i+istart];
			prow_ann = ppdata_ann[i+istart];
			n = 0;
			//inputs
			for (j=0; j<ninput; j++)
			{
