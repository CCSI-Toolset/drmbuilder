...
static PyObject* db_generate_drm(PyObject* self, PyObject* args)
{
	...
	PyObject* obj_td_size;				//training data size tuple, number of pairs in each set, total is npairs
	...
	if (!PyArg_ParseTuple(args, "iiiOOO", &imodel_type, &ninput, &noutput, &obj_td_size, &obj_td, &obj_drm_opt))
		return NULL;
	drmct.SetModelType(imodel_type);
	drmct.SetNumberOfInputs(ninput);
	drmct.SetNumberOfOutputs(noutput);
	//get the training data size including number of sets
	seq = PySequence_Fast(obj_td_size, "expect training data size vector");
	if (!seq)
		return NULL;
	nset = PySequence_Size(obj_td_size);
	if (nset<0)
		return NULL;
	v_npair.resize(nset);
	npair = 0;
	...
	drmct.data_varied.SetNset(nset);
	drmct.data_varied.SetVnpair(v_npair);
