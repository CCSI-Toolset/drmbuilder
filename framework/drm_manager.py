...
class DRMManager(object):
    ...
    def __init__(self):
        ...
        self.bread_custom_train = False             # true if custum training data have been read
        self.bread_custom_valid = False             # true if custum validation data have been read
        ...
        self.acm_result_train_set_size = None       # List of the number of data points for each set of training data, if calculated by ACM, there is only one set
        self.acm_result_valid = None                # ACM results for validation with ramp points filtered out
        self.acm_result_valid_set_size = None       # List of the number of data points for each set of validation data, if calculated by ACM, there is only one set
        ...
        self.drm_container = None                   # DRM container
        bsinter_configured = False                  # ConsoleSinter.exe configured in C:\ProgramData
        self.simsinter_exe = None
        # check if ProgramData folder contains CCSI/DRMBuilder/Config.txt
        config_file = r"C:\ProgramData\CCSI\DRMBuilder\config.txt"
        if os.path.exists(config_file):
            with open(config_file, "r") as f:
                lines = f.readlines()
            # check if the file name is ConsoleSinter.exe
            basename = os.path.basename(lines[0])
            if basename=="ConsoleSinter.exe":
                if os.path.exists(lines[0]):
                    print "ConsoleSinter.exe configured and found on your computer."
                    bsinter_configured = True
                    self.simsinter_exe = lines[0]
                else:
                    print "ConsoleSinter.exe is not found on your computer!"
            else:
                print "C:\ProgramData\CCSI\DRMBuilder\config.txt should contain a valid full path to ConsoleSinter.exe!"
        else:
            simsinter_exe_default = r"C:\Program Files (x86)\CCSI\SimSinter\ConsoleSinter.exe"
            if os.path.exists(simsinter_exe_default):
                print "ConsoleSinter.exe is found on your computer and is configured for DRMBuilder"
                self.simsinter_exe = simsinter_exe_default
                # check if ProgramData\CCSi\DRMBuilder fold exists
                config_path = os.path.dirname(config_file)
                if not os.path.exists(config_path):
                    os.makedirs(config_path)
                with open(config_file, "w") as f:
                    f.write(self.simsinter_exe)
                bsinter_configured = True
        if bsinter_configured==False:
            # ask user to configure ConsoleSinter.exe
            if (QMessageBox.question(None, "SimSinter Installation Status", "Has ConsoleSinter.exe been installed?", QMessageBox.Yes, QMessageBox.No)==QMessageBox.Yes):
                file_name, file_filter = QFileDialog.getOpenFileName(None, "Select SimSinter File", None, "ConsoleSinter (*.exe)")
                if file_name:
                    print 'Configured executable file to launch dynamic simulation is:\n' + file_name
                    self.simsinter_exe = file_name
                    # check if ProgramData\CCSi\DRMBuilder fold exists
                    config_path = os.path.dirname(config_file)
                    if not os.path.exists(config_path):
                        os.makedirs(config_path)
                ...
                print "Please install ConsoleSinter.exe through CCSI's SimSinter package."
                exit(0)
    ...
    def to_dictionary(self):
        ...
        dict_data['bread_custom_train'] = self.bread_custom_train
        dict_data['bread_custom_valid'] = self.bread_custom_valid
        ...
        if self.brun_train:
            ...
            dict_data['acm_result_train_set_size'] = self.acm_result_train_set_size
        if self.brun_valid:
            ...
            dict_data['acm_result_valid_set_size'] = self.acm_result_valid_set_size
    ...
    def from_dictionary(self, dict_data):
        ...
        self.bread_custom_train = dict_data['bread_custom_train']
        self.bread_custom_valid = dict_data['bread_custom_valid']
        ...
        if self.brun_train:
            ...
            self.acm_result_train_set_size = dict_data['acm_result_train_set_size']
        if self.brun_valid:
            ...
            self.acm_result_valid_set_size = dict_data['acm_result_valid_set_size']
        if self.bpredict_train:
    ...
    def parse_simsinter_config(self):
        ...
        if not dict_data['settings']['RunMode']['default']=='Dynamic':
            ...
            return False
        if not os.path.isfile(dict_data['model']['file']):
    ...
    def parse_custom_train_data(self, csvfile):
        csvdata = list()
        f = open(csvfile,'rt')
        try:
            reader = csv.reader(f)
            ...
            f.close()
        nline = len(csvdata)
        #Row 1: sampling time and time unit
        self.acm_input.dt_sampling = float(csvdata[0][0])
        self.acm_input.dt_min_solver = self.acm_input.dt_sampling
        self.acm_input.unit_time = csvdata[0][1]
        #Row 2: numbers of input and output variables
        ninput = int(csvdata[1][0])
        noutput = int(csvdata[1][1])
        ninout = ninput + noutput
        if ninout!=len(csvdata[2]):
            QMessageBox.warning(None, 'Warning', 'Mismatch in the numbers of input and output variables')
            return False
        del self.input_list_all[:]
        del self.output_list_all[:]
        #Row 3: input variable sinter keys, assume all variables are selected (bvaried=True), no ramp (bramp=False, ramp_rate=1)
        #Row 4: input variable names, corresponding to ACM variable path
        #Row 5: input variable description
        #Row 6: units of input variables
        for i in xrange(ninput):
            input_var = InputVariable()
            input_var.key_sinter = csvdata[2][i]
            input_var.name = csvdata[3][i]
            input_var.desc = csvdata[4][i]
            input_var.unit = csvdata[5][i]
            input_var.bvaried = True
            input_var.bramp = False
            input_var.ramp_rate = 1.0
            self.input_list_all.append(input_var)
        for i in xrange(noutput):
            output_var = OutputVariable()
            output_var.key_sinter = csvdata[2][ninput+i]
            output_var.name = csvdata[3][ninput+i]
            output_var.desc = csvdata[4][ninput+i]
            output_var.unit = csvdata[5][ninput+i]
            output_var.bvaried = True
            self.output_list_all.append(output_var)
        #read number of data points for each set of data
        #Row 7: number of data sets
        nset = int(csvdata[6][0])
        nrow_heading = 7
        for i in xrange(nset):
            self.acm_result_train_set_size = [None]*nset
        nsum = 0
        for i in xrange(nset):
            self.acm_result_train_set_size[i] = int(csvdata[nrow_heading][0])
            nrow_heading += 1
            nsum += self.acm_result_train_set_size[i]
        npair = nline - nrow_heading
        if nsum!=npair:
            QMessageBox.warning(None, 'Warning', 'Mismatch in the numbers of data points')
            return False
        #read actual data
        self.acm_result_train = [None]*ninout
        for i in xrange(ninout):
            self.acm_result_train[i] = [None]*npair
            for j in xrange(npair):
                self.acm_result_train[i][j] = float(csvdata[nrow_heading+j][i])
        #calculate lower and upper limits
        for i in xrange(ninput):
            self.input_list_all[i].xlower = min(self.acm_result_train[i])
            self.input_list_all[i].xupper = max(self.acm_result_train[i])
            self.input_list_all[i].xdefault = (self.input_list_all[i].xlower + self.input_list_all[i].xupper)/2.0
        for i in xrange(noutput):
            self.output_list_all[i].xdefault = (min(self.acm_result_train[i+ninput]) + max(self.acm_result_train[i+ninput]))/2.0
        #update selected list as all variable list
        self.update_selected_input_list()
        self.update_selected_output_list()
        return True

    def parse_custom_valid_data(self, csvfile):
        csvdata = list()
        f = open(csvfile,'rt')
        try:
            reader = csv.reader(f)
            ...
            f.close()
        nline = len(csvdata)
        #skip first 6 rows since they have been set in training data
        #Row 1: sampling time and time unit
        #Row 2: numbers of input and output variables
        #Row 3: input variable sinter keys, assume all variables are selected (bvaried=True), no ramp (bramp=False, ramp_rate=1)
        #Row 4: input variable names, corresponding to ACM variable path
        #Row 5: input variable description
        #Row 6: units of input variables
        ninput = len(self.input_list_all)
        noutput = len(self.output_list_all)
        ninout = ninput + noutput
        if ninout!=len(csvdata[2]):
            QMessageBox.warning(None, 'Warning', 'Mismatch in the numbers of input and output variables')
            return False
        ...
        nrow_heading = 7
        for i in xrange(nset):
            self.acm_result_valid_set_size = [None]*nset
        nsum = 0
        for i in xrange(nset):
            self.acm_result_valid_set_size[i] = int(csvdata[nrow_heading][0])
            nrow_heading += 1
            nsum += self.acm_result_valid_set_size[i]
        npair = nline - nrow_heading
        if nsum!=npair:
            QMessageBox.warning(None, 'Warning', 'Mismatch in the numbers of data points')
            return False
        #read actual data
        self.acm_result_valid = [None]*ninout
        for i in xrange(ninout):
            self.acm_result_valid[i] = [None]*npair
            for j in xrange(npair):
                self.acm_result_valid[i][j] = float(csvdata[nrow_heading+j][i])
        return True
    ...
    def run_acm_training(self):
        ...
        # only one set of data when simulated by ACM
        self.acm_result_train_set_size = [None]*1
        self.acm_result_train_set_size[0] = len(self.acm_result_train[0])
    ...
    def run_acm_validation(self):
        ...
        # only one set of data when simulated by ACM
        self.acm_result_valid_set_size = [None]*1
        self.acm_result_valid_set_size[0] = len(self.acm_result_valid[0])
    ...
    def generate_drm(self):
        ...
        acm_result_train_set_size_t = tuple(self.acm_result_train_set_size)
        rst = DRMTraining.generate_drm(self.imodel_type, ninput, noutput, acm_result_train_set_size_t, varied_data_train_t, drm_opt_t)
    ...
    def predict_train(self):
        ...
        if self.imodel_type == 0:   # DABNet
            self.drm_prediction_train = self.drm_container.predict_dabnet(varied_data, self.ipredict_opt_dabnet, self.acm_result_train_set_size)
        else:                       # NARMA
    ...
    def predict_valid(self):
        # assume always single set of data
        varied_data = self.get_varied_data_from_acm_result(self.acm_result_valid)
        if self.imodel_type == 0:   # DABNet
            self.drm_prediction_valid = self.drm_container.predict_dabnet(varied_data, self.ipredict_opt_dabnet, self.acm_result_valid_set_size)
        else:                       # NARMA
