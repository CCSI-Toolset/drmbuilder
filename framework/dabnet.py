...
class DABNet(object):
    ...
    def predict(self, u, ipredict_opt, vnpair):
        ...
        # vnpair: sequence of number of data point in each data set
        ...
        if ipredict_opt == 1:   # reduced model
            ...
            for iset in xrange(nset):
                npair = vnpair[iset]
                for j in xrange(self.ninput):
                    self.state_space_red[j].init_state_vector_to_steady_state(u[istart][j])
                for i in xrange(npair):
                    ...
                    for j in xrange(self.ninput):
                        for k in xrange(self.state_space_red[j].nstate):
                            ann_input[n] = self.state_space_red[j].x[k][0]
                            n += 1
                        self.state_space_red[j].calc_next_state_vector(u[i+istart][j])
                    # always scaling input but not scaling output
                    y[i+istart] = self.ann_red.predict(ann_input, 1, 0)[0]
                istart += npair
        else:                   # Laguerre model
            ann_input = [None]*self.ann_lag.ninput
            istart = 0
            for iset in xrange(nset):
                npair = vnpair[iset]
                for j in xrange(self.ninput):
                    self.state_space_lag[j].init_state_vector_to_steady_state(u[istart][j])
                for i in xrange(npair):
                    n = 0
                    for j in xrange(self.ninput):
                        for k in xrange(self.state_space_lag[j].nstate):
                            ann_input[n] = self.state_space_lag[j].x[k][0]
                            n += 1
                        self.state_space_lag[j].calc_next_state_vector(u[i+istart][j])
                    # always no scaling of input and output
                    y[i+istart] = self.ann_lag.predict(ann_input, 1, 0)[0]	#debug default 0 for scaling
                istart += npair
        return y
