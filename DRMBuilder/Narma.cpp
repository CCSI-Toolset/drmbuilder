...
void CNarma::TrainNeuralNetwork()
{
	...
	ann.SetMaximumNumverOfEpoches(nmax_iter_bp);
...
void CNarma::Write(FILE* pf)
{
	...
	fwrite(&nhistory,sizeof(int),1,pf);
	fwrite(&nneuron_hid,sizeof(int),1,pf);
	fwrite(&nmax_iter_bp,sizeof(int),1,pf);
	ann.Write(pf);
	//no need to save ann_data
}

void CNarma::Read(FILE* pf)
{
	//ipredict_opt added in version 1 or later
	int iversion;
	...
	fread(&nhistory,sizeof(int),1,pf);
	fread(&nneuron_hid,sizeof(int),1,pf);
	fread(&nmax_iter_bp,sizeof(int),1,pf);
	ann.Read(pf);
}