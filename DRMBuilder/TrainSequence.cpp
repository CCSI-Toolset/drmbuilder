...
void CTrainSequence::Write(FILE* pf)
{
	int i;
	int iversion = 0;
	bool bpistart = pistart!=NULL;
	bool bplhs = plhs!=NULL;
	fwrite(&iversion,sizeof(int),1,pf);
	fwrite(&ndim,sizeof(int),1,pf);
	fwrite(&npoint,sizeof(int),1,pf);
	fwrite(&nduration,sizeof(int),1,pf);
	fwrite(&bpistart,sizeof(bool),1,pf);
	if (bpistart)
		fwrite(pistart,sizeof(int),ndim,pf);
	fwrite(&bplhs,sizeof(bool),1,pf);
	if (bplhs)
	{
		for (i=0; i<nduration; i++)
			plhs[i].Write(pf);
...
void CTrainSequence::Read(FILE* pf)
{
	DeleteArray();
	int i;
	int iversion;
	...
	fread(&bpistart,sizeof(bool),1,pf);
	if (bpistart)
	{
		AllocateArray();
		fread(pistart,sizeof(int),ndim,pf);
	}
	fread(&bplhs,sizeof(bool),1,pf);
	if (bplhs)
	{
		for (i=0; i<nduration; i++)
			plhs[i].Read(pf);
