...
void CLHS::Write(FILE* pf)
{
	...
	fwrite(&bppx,sizeof(bool),1,pf);
	if (bppx)
	{
		for (i=0; i<npoint; i++)
			fwrite(ppx[i],sizeof(int),ndim,pf);
	}
}

void CLHS::Read(FILE* pf)
{
	int i;
	int iversion;
	bool bppx;
	...
	fread(&bppx,sizeof(bool),1,pf);
	if (bppx)
	{
		AllocateSampleArray();
		for (i=0; i<npoint; i++)
			fread(ppx[i],sizeof(int),ndim,pf);
	}
}