...
#include "DRMManager.h"		//for log message
...
void CNeuralNetwork::TrainDataSetBatch(int npair, double** ppinout)
{
	...
	for (iepoch=0; iepoch<nepoch_max; iepoch++)
	{
		...
		if (iepoch%1000==0)
		{
			str.Format("At Epoch=%d, Average Error=%lg.\n",iepoch,error_batch/npair);
...
void CNeuralNetwork::GetHessianNonzeroIndices(int* pirow, int* picol)
{
	...
	if (n!=nconnection*(1+nconnection)/2 - noutput*(noutput-1)/2*pnneuron_layer[1]*pnneuron_layer[1])
		AfxMessageBox("incorrect nonzero emement count!");
}
...
void CNeuralNetwork::Write(FILE* pf)
{
	int iversion = 0;
	bool bint = pnneuron_layer!=NULL;
	bool bdouble = py!=NULL;
	fwrite(&iversion,sizeof(int),1,pf);
	fwrite(&irand_seed,sizeof(int),1,pf);
	fwrite(&itrain_method,sizeof(int),1,pf);
	fwrite(&ninput,sizeof(int),1,pf);
	fwrite(&noutput,sizeof(int),1,pf);
	fwrite(&nneuron,sizeof(int),1,pf);
	fwrite(&nconnection,sizeof(int),1,pf);
	fwrite(&nlayer,sizeof(int),1,pf);
	fwrite(&iactivation_hidden,sizeof(int),1,pf);
	fwrite(&iactivation_output,sizeof(int),1,pf);
	fwrite(&nepoch_max,sizeof(int),1,pf);
	fwrite(&weight_init,sizeof(double),1,pf);
	fwrite(&rate_learn,sizeof(double),1,pf);
	fwrite(&coeff_momentum,sizeof(double),1,pf);
	fwrite(&delta_min,sizeof(double),1,pf);
	fwrite(&delta_max,sizeof(double),1,pf);
	fwrite(&delta0,sizeof(double),1,pf);
	fwrite(&eta_minus,sizeof(double),1,pf);
	fwrite(&eta_plus,sizeof(double),1,pf);
	fwrite(&steepness_hidden,sizeof(double),1,pf);
	fwrite(&steepness_output,sizeof(double),1,pf);
	fwrite(&error_pair,sizeof(double),1,pf);
	fwrite(&error_batch,sizeof(double),1,pf);
	fwrite(&error_batch_old,sizeof(double),1,pf);
	fwrite(&error_stop,sizeof(double),1,pf);
	fwrite(&bint,sizeof(bool),1,pf);
	if (bint)
		fwrite(pnneuron_layer,sizeof(int),nlayer+nneuron+3*nconnection,pf);
	fwrite(&bdouble,sizeof(bool),1,pf);
	if (bdouble)
		fwrite(py,sizeof(double),nneuron*4 + nconnection*5 + (ninput+noutput)*2,pf);
}

void CNeuralNetwork::Read(FILE* pf)
{
	int iversion;
	bool bint;
	bool bdouble;
	fread(&iversion,sizeof(int),1,pf);
	fread(&irand_seed,sizeof(int),1,pf);
	fread(&itrain_method,sizeof(int),1,pf);
	fread(&ninput,sizeof(int),1,pf);
	fread(&noutput,sizeof(int),1,pf);
	fread(&nneuron,sizeof(int),1,pf);
	fread(&nconnection,sizeof(int),1,pf);
	fread(&nlayer,sizeof(int),1,pf);
	fread(&iactivation_hidden,sizeof(int),1,pf);
	fread(&iactivation_output,sizeof(int),1,pf);
	fread(&nepoch_max,sizeof(int),1,pf);
	fread(&weight_init,sizeof(double),1,pf);
	fread(&rate_learn,sizeof(double),1,pf);
	fread(&coeff_momentum,sizeof(double),1,pf);
	fread(&delta_min,sizeof(double),1,pf);
	fread(&delta_max,sizeof(double),1,pf);
	fread(&delta0,sizeof(double),1,pf);
	fread(&eta_minus,sizeof(double),1,pf);
	fread(&eta_plus,sizeof(double),1,pf);
	fread(&steepness_hidden,sizeof(double),1,pf);
	fread(&steepness_output,sizeof(double),1,pf);
	fread(&error_pair,sizeof(double),1,pf);
	fread(&error_batch,sizeof(double),1,pf);
	fread(&error_batch_old,sizeof(double),1,pf);
	fread(&error_stop,sizeof(double),1,pf);
	fread(&bint,sizeof(bool),1,pf);
	if (bint)
	{
		AllocateIntArrays();
		fread(pnneuron_layer,sizeof(int),nlayer+nneuron+3*nconnection,pf);
	}
	fread(&bdouble,sizeof(bool),1,pf);
	if (bdouble)
	{
		AllocateDoubleArrays();
		fread(py,sizeof(double),nneuron*4 + nconnection*5 + (ninput+noutput)*2,pf);
	}
}